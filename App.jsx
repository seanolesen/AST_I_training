...
const TOPICS = {
  ...
  travel:   "Travel & Human Factors",
};

// ---- Question bank --------------------------------------------------
// type: "mc" | "tf" | "match"
// diff: "easy" | "moderate" | "hard"
// ---- Difficulty blend weights --------------------------------------
// A difficulty setting weights the draw across the whole bank rather
// than hard-filtering, so sets stay varied even in small topics.
const DIFF_WEIGHTS = {
    ...
    <div style={shell}>
      <div style={card}>
        ...
        <p style={{ color: C.textDim, fontSize: 13.5, lineHeight: 1.5, margin: "0 0 20px" }}>
          Practice questions across the Avalanche Skills Training 1 curriculum — terrain, snowpack, weather, the forecast, trip planning, companion rescue, and human factors.
        </p>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {[{ value: "all", label: "All topics" }, ...Object.entries(TOPICS).map(([k, v]) => ({ value: k, label: v }))].map((o) => {
...
function Quiz({ questions, settings, recording, onFinish }) {
  ...
  const topicLabel = TOPICS[q.topic];
...
function Results({ graded, settings, recording, onRetry, onNew, onHome }) {
  ...
  const card = { maxWidth: 540, margin: "0 auto" };
  const hitTarget = pct >= PRACTICE_TARGET;
    ...
    <div style={shell}>
      <div style={card}>
        ...
        <div style={{ fontSize: 12.5, color: C.textMute, marginBottom: 4 }}>
          {settings.difficulty} difficulty · {settings.topic === "all" ? "all topics" : TOPICS[settings.topic]}
            ...
            return (
              <div key={t} style={{ marginBottom: 9 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, marginBottom: 3 }}>
                  <span style={{ color: C.textDim }}>{TOPICS[t]}</span>
              ...
              <div>
                ...
                {analysis.weak.map((w) => (
                  <div key={w.topic} style={{ fontSize: 12.5, color: C.snow, marginBottom: 2 }}>
                    · {TOPICS[w.topic]} — {Math.round(w.acc * 100)}% <span style={{ color: C.textMute }}>({w.n} seen)</span>
                  </div>
                ))}
              </div>
            )}
...
export function Ast1App({ onHome }) {
  ...
  const start = useCallback(() => {
    const qs = weightedSample(poolFor(settings.topic), settings.count, settings.difficulty);
